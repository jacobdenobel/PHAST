...
def lgf(
    ...
    base_level = parameters.amp.sat_level / from_dB(parameters.amp.dynamic_range_dB)
    alpha = lgf_alpha(Q, base_level, parameters.amp.sat_level)
    
    # Scale the input between base_level and sat_level:
    # Find all the inputs that are above sat_level (i.e. r > 1) 
    r = np.clip((channel_power - base_level) / (parameters.amp.sat_level - base_level), None, 1)
